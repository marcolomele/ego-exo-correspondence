"""
Pre-extract DINOv3 features for a random sample of frames targeting ~13K ego-exo pairs.
...
class DINOFeatureExtractor:
    """Extract dense features from images using DINOv3."""
    ...
    def __init__(self, model_name, patch_size, device):
        ...
        print(f"Loading DINOv3 model from {dinov3_dir}")
        self.model = torch.hub.load(dinov3_dir, model_name, source='local', weights=weights_path).to(device)
        self.model.eval()
...
def precompute_features(args):
    """Main function to precompute and save DINOv3 features."""
    ...
    extractor = DINOFeatureExtractor('dinov3_vits16', args.patch_size, device)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Pre-extract DINOv3 features for O-MaMa dataset")
